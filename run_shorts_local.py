"""
run_shorts_local.py — GUI pt Shorts/TikTok cu Reddit stories + Medal gaming clips.
...
def refresh_proxies():
    ...
    if not os.path.exists(proxy_script):
        print("[run_shorts_local] proxy_api.py not found, skipping")
        return
    ...
    if os.path.exists(proxies_file):
        ...
        if age < 600:
            ...
            print(f"[run_shorts_local] proxies.txt fresh ({age:.0f}s, {count} proxies). Skip.")
            return

    print("[run_shorts_local] Refreshing proxies...")
    try:
        subprocess.run([sys.executable, proxy_script], check=True,
                       capture_output=True, text=True, timeout=120, cwd=root_dir)
    except Exception as e:
        print(f"[run_shorts_local] Proxy refresh failed: {e}, continuing")


if __name__ == "__main__":
    refresh_proxies()
    from gui.main_shorts_local import run
    run()
